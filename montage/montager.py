...
class Montager:
    def __init__(self, celldir=None, verbose=True, use_config=True):
    ...
    def run(self):
        ...
        self.videos = list(celldir.glob("video_???.ma"))
        self.images = list(celldir.glob("image_???.tif"))
    ...
    def get_images(self, celldir):
        self.celldir = celldir
        self.images = Path(celldir).glob("/image*.tif")
    ...
    def set_image(self, celldir, imagename):
        self.celldir = celldir
        self.images = Path(celldir).glob(imagename)
        ...
        if window == "mpl":
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    print("image shape: ", thisimg.shape)
    ...
    def merge_flattened_videos(
        ...
        for i, v in enumerate(metadata):
            ...
            extentx = [
                vt["pos"][0] + float(vr[0]) * vt["scale"][0],
            ...
            if self.verbose:
                print("extent x: ", extentx)
                print("extent y: ", extenty)
        ...
        if self.verbose:
            ...
            print(
                "minx: %15g  maxx: %15g miny: %g  maxy: %g"
                % (minx, maxx, miny, maxy)
            )
            print(
                "spanx: %f  spany: %f" % ((maxx - minx), (maxy - miny))
            )
        ...
        for i, vdata in enumerate(imagedata):  # place the images onto the big map
            ...
            if delx > 0 or dely > 0:
                print("    **** Padding *****")
                self.merged_image = np.pad(
                    self.merged_image,
                    pad_width=((0, delx), (0, dely + 1)),
                    mode="constant",
                    constant_values=bkgd,
                )
                self.n_images = np.pad(
                    self.n_images,
                    pad_width=((0, delx), (0, dely + 1)),
                    mode="constant",
                    constant_values=bkgd,
            ...
            if bigpoly.intersects(data_poly):
                ...
                off = {"angle": 0.0, "tvec": [0, 0]}
                if (
                    mosaic_data is None
                    and np.sum(self.n_images[b[0] : b[2], b[1] : b[3]] > 0) > thr
                ):  # exceed threshold of overlap, try alignment against existing
                    # print('threshold: ', np.sum(self.n_images[b[0]:b[2], b[1]:b[3]] > 0), thr, target_shape)
                    i_m = self.merged_image[
                        b[0] : b[2], b[1] : b[3]
                    ]  # get intersection in image
                    x0 = b[0] - ix
                    x1 = b[2] - ix
                    y0 = b[1] - iy
                    y1 = b[3] - iy
                ...
                if (
                    off["angle"] == 0
                    and abs(off["tvec"][0]) < 200
                    and abs(off["tvec"][1]) < 200
                ):  # rotation and big translation is not allowed
                    offv = off["tvec"]
                    ix = int(ix - offv[1])
                    ixw = int(ixw - offv[1])
                    y0 = int(iyw - offv[0])
            ...
            self.n_images[
                ix:ixw, yseq
            ] += 1.0  # keep track of how many planes were added here
        ...
        self.image_boundary = [
            self.bigpoly.bounds[0] * vscalex,
            self.bigpoly.bounds[1] * vscaley,
            self.bigpoly.bounds[2] * vscalex,
            self.bigpoly.bounds[3] * vscaley,
        ]

        self.n_images[self.n_images == 0.0] = np.nan  # trick for avoiding divide by 0
        ...
        print(
            "image minm: ",
            np.min(self.merged_image),
            "  max: ",
            np.max(self.merged_image),
        )
...
def main():
    ...
    M = Montager(verbose=True)  # , celldir=Path(sel.fileName))
    M.run()
